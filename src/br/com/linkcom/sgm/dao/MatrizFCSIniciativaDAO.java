...
public class MatrizFCSIniciativaDAO extends GenericDAO<MatrizFCSIniciativa> {
	
	@Override
	public void updateSaveOrUpdate(SaveOrUpdateStrategy save) {
		save.saveOrUpdateManagedDeleteFirst("listaMatrizFcsIniciativaFator");
	}
	
	public List<MatrizFCSIniciativa> findByMatrizFCS(MatrizFCS matrizFCS){
		return query()
				.joinFetch("matrizFCSIniciativa.listaMatrizFcsIniciativaFator matrizFcsIniciativaFator")
				.joinFetch("matrizFcsIniciativaFator.itemFatorAvaliacao itemFatorAvaliacao")
				.where("matrizFCSIniciativa.matrizFCS=?",matrizFCS)
				.orderBy("matrizFCSIniciativa.id, matrizFcsIniciativaFator.id")
				.list();
	}
	...
	public List<MatrizFCSIniciativa> findPrioritariasByUnidadeGerencial(UnidadeGerencial unidadeGerencial) {
		if (unidadeGerencial == null || unidadeGerencial.getId() == null) {
			return new ArrayList<MatrizFCSIniciativa>();
		}
		return 
			query()
				.select("matrizFCSIniciativa.id, matrizFCSIniciativa.descIniciativa, " +
						"objetivoMapaEstrategico.id")
				.join("matrizFCSIniciativa.matrizFCS matrizFCS")
				.join("matrizFCS.objetivoMapaEstrategico objetivoMapaEstrategico")
				.where("matrizFCSIniciativa.matrizFCS.unidadeGerencial = ?", unidadeGerencial)
				.where("matrizFCSIniciativa.prioritaria = ?", Boolean.TRUE)
				.orderBy("matrizFCSIniciativa.descIniciativa")
				.list();
	}
	...
	public MatrizFCSIniciativa loadWithPlanoAcao(MatrizFCSIniciativa matrizFCSIniciativa) {
		return query()
			.from(MatrizFCSIniciativa.class)
			.leftOuterJoinFetch("matrizFCSIniciativa.listaPlanoAcao planoAcao")
			.entity(matrizFCSIniciativa)
			.unique();
	}
}
