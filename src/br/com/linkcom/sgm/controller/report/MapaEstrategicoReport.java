 ...
 */
@Bean
@Controller(path="/sgm/report/MapaEstrategico", authorizationModule=ReportAuthorizationModule.class)
public class MapaEstrategicoReport extends GeplanesReport<MapaEstrategicoReportFiltro>{
	...
	@Override
	@Action("gerar")
	public IReport createReportGeplanes(WebRequestContext request, MapaEstrategicoReportFiltro filtro) throws Exception {
		
		filtro.setUnidadeGerencial(unidadeGerencialService.load(filtro.getUnidadeGerencial()));
		if (!Boolean.TRUE.equals(filtro.getUnidadeGerencial().getPermitirMapaEstrategico())) {
			throw new GeplanesException("Não é permitida a exibição do mapa estratégico para essa unidade gerencial.");
		}
		...
		return unidadeGerencialService.createMapaEstrategicoReport(filtro);
	}
	
	@Override
	@DefaultAction
	public ModelAndView doFiltro(WebRequestContext request,MapaEstrategicoReportFiltro filtro)throws ResourceGenerationException {
